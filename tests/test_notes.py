...
def get_auth_token(test_client, seed_roles):
...
def test_create_note(test_client, seed_roles):
    token = get_auth_token(test_client, seed_roles)
    response = create_note(test_client, token)
    
    assert response.status_code == 201
    assert response.json()["title"] == "My Note"

def test_get_all_notes(test_client, seed_roles):
    token = get_auth_token(test_client, seed_roles)
    response = create_note(test_client, token)
...
def test_get_specific_note(test_client, seed_roles):
    token = get_auth_token(test_client, seed_roles)
...
def test_update_note(test_client, seed_roles):
    token = get_auth_token(test_client, seed_roles)
    create_response = response = create_note(test_client, token)
...
def test_delete_note(test_client, seed_roles):
    token = get_auth_token(test_client, seed_roles)
    create_response = response = create_note(test_client, token)
